...
> **Note on JSON field names:** The actual `quran_id.json` from risan/quran-json uses different field names than originally planned. Use the mapping below:
> - Surah Latin name → `map['transliteration']` (not `map['name']`)
> - Surah Arabic name → `map['name']` (not `map['name_arabic']`)
> - Verse count → `map['total_verses']` (not `map['verses_count']`)
> - Revelation type → `map['type']` with values `'meccan'`/`'medinan'` (not `map['revelation_place']` / `'mecca'`)
> - Juz → **not in JSON**, use the hardcoded `_juzForSurah()` lookup table below.
...
> **Note on verse numbering:** The `quran_id.json` dataset uses sequential global IDs for verses. When seeding, the `number` field (verse number within surah) is computed as the index within the surah's verses array + 1.
